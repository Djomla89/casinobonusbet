...
function post_custom_fields() {
	...
	foreach($post_custom_fields as $meta_box) {
		...
		if($meta_box_value == "")
			...
			echo'<strong style="float:left;">'.$meta_box['title'].'</strong>'.'<br/>';
			echo'<textarea rows="4" cols="160" name="'.$meta_box['name'].'_value" value="'.attribute_escape($meta_box_value).'" style="margin-top:10px;">'.attribute_escape($meta_box_value).'</textarea><br />';
			echo '</p>';
...
function create_meta_box2() {
	global $theme_name;
		if ( function_exists('add_meta_box') ) {
			add_meta_box( 'new-meta-boxes', 'Additional Information', 'page_custom_fields', 'page', 'normal', 'high' );
	}
}
